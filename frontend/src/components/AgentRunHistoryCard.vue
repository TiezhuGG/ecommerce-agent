...
<template>
  <section class="panel p-6">
    <div class="flex items-start justify-between gap-4">
      <div>
        ...
        <p class="muted-copy mt-2">
          这里展示最近写入数据库的 Agent 运行摘要，用来确认日志是否真正落库，以及每次调用大致走了哪条链路。
        </p>
      </div>
    ...
    <template v-if="history">
      ...
      >
        当前未启用数据库日志存储，所以这里只有空列表。配置可用的 `DATABASE_URL` 后，这里会显示最近的 Agent 运行历史。
      </div>
      ...
      <div v-else-if="history.items.length" class="mt-5 grid gap-3">
        ...
        >
          ...
          <p class="mt-3 text-xs text-slate-500">
            {{ formatTimestamp(item.createdAt) }} · {{ item.runId }}
          </p>
